...
transpose lines =
    let
        regularLines : List String
        regularLines =
            let
                l : Int
                l =
                    longestStringLength lines
            in
            List.map (String.padRight l ' ') (List.map (String.replace " " "/") lines)
    in
    List.map (String.replace "/" " ") (List.map String.trimRight (transposeRegular regularLines))
...
transposeRegular lines =
    case sequence (List.map String.uncons lines) of
        ...
        Just l ->
            case List.unzip l of
                ( firsts, rests ) ->
                    String.fromList firsts :: transposeRegular rests
