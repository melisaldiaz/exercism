...
nthDatum :: LinkedList a -> Int -> Maybe a
...
specs = do
            ...
            it "fromList" $ do
                ...
                datum fl1                `shouldBe` Just 1
                isNil (next fl1)         `shouldBe` True
                datum fl21               `shouldBe` Just 2
                datum (next fl21)        `shouldBe` Just 1
                isNil (next $ next fl21) `shouldBe` True

            it "reverseLinkedList" $ do
                isNil (reverseLinkedList nil) `shouldBe` True
                datum r1                      `shouldBe` Just 1
                isNil (next r1)               `shouldBe` True
                datum r12                     `shouldBe` Just 1
                datum (next r12)              `shouldBe` Just 2
                isNil (next $ next r12)       `shouldBe` True
                datum r123                    `shouldBe` Just 1
                datum (next r123)             `shouldBe` Just 2
                datum (next $ next r123)      `shouldBe` Just 3
            ...
            it "arbitrary datum" $
                property $ \(xs :: [Int]) ->
                  let ll = fromList xs
                      sameNthDatum n = nthDatum ll n == Just (xs !! n)
